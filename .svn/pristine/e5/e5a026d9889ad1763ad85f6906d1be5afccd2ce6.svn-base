...
class MenberController extends Controller {
    ...
    public function mlist()
    {
		...
		$field = $tq.'userinfo.username as username,'.$tq.'userinfo.utel as utel,'.$tq.'userinfo.utime as utime,'.$tq.'userinfo.uid as usid,'.$tq.'userinfo.otype as otype,'.$tq.'userinfo.portrait as portrait,'.$tq.'userinfo.ustatus as ustatus,'.$tq.'userinfo.oid as oid,'.$tq.'userinfo.address as address,'.$tq.'userinfo.portrait as portrait,'.$tq.'userinfo.lastlog as lastlog,'.$tq.'userinfo.managername as managername,'.$tq.'userinfo.comname as comname,'.$tq.'userinfo.comqua as comqua,'.$tq.'userinfo.rebate as rebate,'.$tq.'userinfo.feerebate as feerebate,'.$tq.'accountinfo.balance as balance';
		...
		if($step == "search"){
			...
			//分页
			$count = $user->where('ustatus=0 and otype=2 and wxtype=0')->count();
	        $pagecount = 10;
		...
		//统计
		//用户数量
    	$userCount = $user->where('ustatus=0 and otype=2 and wxtype=0')->count();
		//交易手数
...
//		$userinfo = M('userinfo');
        ...
        $userinfo = D('userinfo');
        $uid = $_SESSION['userid'];
        $admin = $userinfo->where('uid='.$uid)->find();
