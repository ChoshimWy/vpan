	...
	public function signin()
	{
	 	if(IS_POST){
			...
			if(empty($result)){
                ...
                if ($result['otype']>=3&&$result['ustatus']==0&&$result['upwd']== md5(I('post.password').$result['utime']))
...
//						$this->success('登录成功', U('Index/index'));
    ...
    */
    ...
    public function ulist()
    {
		...
		$step = I('get.step');
		$field = $tq.'userinfo.username as username,'.$tq.'userinfo.nickname as nickname,'.$tq.'userinfo.uid as usid,'.$tq.'userinfo.utel as utel,'.$tq.'userinfo.address as address,'.$tq.'userinfo.portrait as portrait,'.$tq.'userinfo.utime as utime,'.$tq.'userinfo.oid as oid,'.$tq.'userinfo.managername as managername,'.$tq.'userinfo.lastlog as lastlog,'.$tq.'accountinfo.balance as balance,'.$tq.'userinfo.otype as otype,'.$tq.'userinfo.ustatus as ustatus';
        $bankinfo=M('bankinfo');
		if($step == "search"){
			...
			foreach($ulist as $k => $v){
				...
				$us= $user->where('uid='.$v['oid'])->find();
				$usming =$bankinfo->where('uid='.$v['usid'])->find();
				$ulist[$k]['mingcheng']=$usming['busername'];
				$ulist[$k]['managername']=$us['nickname'];
			...
			$where = $key.$formula;
			//查询用户和账户信息
			$ulist = $user->join($tq.'accountinfo on '.$tq.'userinfo.uid='.$tq.'accountinfo.uid','left')->field($field)->order($tq.'userinfo.uid desc')->select();
		...
		//统计
		//用户数量
    	$userCount = $user->where('ustatus=0 and wxtype=0 and otype!=3')->count();
		//交易手数
	...
	//黑名单操作
	public function henming(){
		$uid=I('get.uid');
		$date['ustatus']=I('get.dtr');
		$str=M('userinfo')->where('uid='.$uid)->save($date);
		if ($str) {
			$this->success("操作成功",U("User/ulist"));
		}else{
			$this->error('操作失败');
		}
	}
	...
	public function recharge(){
		...
		$type_val = I('get.type_val');
		
		if($type_val == 1){
			$wheretype = " bptype='充值' and ";
			
		}elseif($type_val == 2){
			$wheretype = " bptype='提现' and  ";
		}else{
        ...
        $step = I('get.step');
		//查询多条记录
       	$field = $tq.'userinfo.username as username,'.$tq.'userinfo.nickname as nickname,'.$tq.'balance.uid as uid,'.$tq.'balance.balance as balance,'.$tq.'balance.bpid as bpid,'.$tq.'balance.bptype as bptype,'.$tq.'balance.bptime as bptime,'.$tq.'balance.bpprice as bpprice,'.$tq.'balance.remarks as remarks,'.$tq.'balance.isverified as isverified,'.$tq.'accountinfo.balance as balance,'.$tq.'balance.cltime as cltime';
		//过滤搜索
		if($step == "sxsearch"){
			...
			$rechargelist = $balance->join($tq.'userinfo on '.$tq.'balance.uid='.$tq.'userinfo.uid','left')->join($tq.'accountinfo on '.$tq.'balance.uid='.$tq.'accountinfo.uid','left')->field($field)->limit($page->firstRow.','.$page->listRows)->order($tq.'balance.bpid desc')->where($wheretype.' 1 ')->select();
			$this->assign('page',$show);
	...
	public function upbalance(){
		...
		$cltime = time();
		if($isverified=="1"){
			$isver=$balance->where('bpid='.$bpid)->setField(array('isverified'=>'1','remarks'=>$remarks,'cltime'=>$cltime));
			//1是同意,同意后扣去申请的金额
			$date=$accountinfo->where('uid='.$userid)->find();
			$date['balance']=$date['balance']-$rebpprce;
            $accountinfo->where('uid='.$userid)->save($date);
            $balance->where('uid='.$userid)->save($date);
		}else if($isverified=="0"){
			$isver=$balance->where('bpid='.$bpid)->setField(array('isverified'=>'2','remarks'=>$remarks,'cltime'=>$cltime,));
			//2是拒绝
		}else{
			$isver=$balance->where('bpid='.$bpid)->setField(array('isverified'=>'0','remarks'=>$remarks,'cltime'=>$cltime));
			//0是初始值
		}
	...
	function payment(){
        ...
        $step = I('get.step');
		//查询多条记录
       	$field = $tq.'userinfo.username as username,'.$tq.'userinfo.nickname as nickname,'.$tq.'balance.uid as uid,'.$tq.'balance.balance as balance,'.$tq.'balance.bpid as bpid,'.$tq.'balance.bptype as bptype,'.$tq.'balance.bptime as bptime,'.$tq.'balance.bpprice as bpprice,'.$tq.'balance.remarks as remarks,'.$tq.'balance.isverified as isverified,'.$tq.'accountinfo.balance as balance,'.$tq.'balance.cltime as cltime,'.$tq.'balance.state as state,'.$tq.'balance.withdrawal as withdrawal';
		//过滤搜索
		if($step == "sxsearch"){
			...
			//分页
			$count = $balance->count('withdrawal=1');
			//print_r($count);exit;
	        $pagecount = 15;
	        $page = new \Think\Page($count , $pagecount);
	        $page->parameter = $row; //此处的row是数组，为了传递查询条件
			...
			$rechargelist = $balance->join($tq.'userinfo on '.$tq.'balance.uid='.$tq.'userinfo.uid','left')->join($tq.'accountinfo on '.$tq.'balance.uid='.$tq.'accountinfo.uid','left')->field($field)->limit($page->firstRow.','.$page->listRows)->order($tq.'balance.bpid desc')->where($wheretype.' 1 ')->where('withdrawal=1')->select();
			$this->assign('page',$show);
			$this->assign('rechargelist',$rechargelist);			
		...
		$cltime = time();
		if($state=="1"){
			$isver=$balance->where('bpid='.$bpid)->setField(array('state'=>'1'));
			
		}else if($state=="0"){
			$isver=$balance->where('bpid='.$bpid)->setField(array('state'=>'2'));
			//2是拒绝
		}else{
			$isver=$balance->where('bpid='.$bpid)->setField(array('state'=>'0'));
			//0是初始值
		}
    ...
    public function expexcel(){//导出Excel
        $fileName = $_SESSION['username'].date('_YmdHis');
        $xlsModel = D('balance');
        $xlsData  = $xlsModel->where('state=1')->select();
